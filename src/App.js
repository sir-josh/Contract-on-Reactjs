...
class App extends Component {
	...
	async componentDidMount() {
		...
		const previousWinner = await lottery.methods.lastWinner().call();
	...
	hidePickWinner = () => {
		return (
			<div className="ui info message">
				...
				The manager will be the one to shuffle the pool. The lucky winner will
	...
	render() {
		const { manager, players, balancePool, value, message, previousWinner } = this.state;
				...
				<div className="ui fluid container">
					<div className="ui block header very padded text center aligned segment">
						<p className="ui tiny header">
							This lottery is managed by {manager}
						</p>
						<p className="sub header">
							There are currently <b>{players.length}</b> people
							that have entered the pool, competing to win{" "}
							<b>{web3.utils.fromWei(balancePool, "ether")}</b>{" "}
							amount of ether.
						</p>            
            <div className="ui transparent basic secondary segment">
              <div className="ui black label">
                <b>Previous winner:</b>
                <div className="detail">{previousWinner}</div>
              </div>
            </div>
					</div>
				</div>
				...
				<div className="ui container" style={{ marginTop: "3rem" }}>
					...
					{!this.state.currentUserIsManager && this.hidePickWinner()}
				</div>
					...
					<div
						...
						<div className="content">
							<form
								...
								<div
									className="eight wide field centered"
